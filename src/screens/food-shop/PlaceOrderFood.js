...
export default function PlaceOrderFood() {
    ...
    const getGrandTotal = () => {
        ...
        if (parseFloat(totalAmountFood) >= parseFloat(minOrderAmount)) {
    ...
    const handleCustomerOrder = () => {
        if (foodItems.length > 0) {
            ...
            placceOrder(itemOrderObj);
        } else {
            setShowErrorMessage(true);
            setMessage('Please Add Product to bag !!');
        }
    };

    return (
        <View style={{ flex: 1, backgroundColor: '#f1f5f7' }}>
            <HeaderCommon title={'Place Food Order'} />
                ...
                <View style={{ flex: 1, alignItems: 'center' }}>
                    <View style={{
                        ...
                        <View style={{ flex: 1, paddingTop: 5, paddingBottom: 5 }}>
                            <Text style={{ color: '#FFF', fontSize: 15 }}>{foodStoreInfo?.shop_notice}</Text>
                        </View>
                    </View>
            ...
            <View style={{ flex: 1, justifyContent: 'space-around', marginHorizontal: 30 }}>
