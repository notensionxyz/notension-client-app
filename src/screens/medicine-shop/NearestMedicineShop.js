...
import { Dimensions, FlatList, Image, Text, BackHandler, View } from "react-native";
...
function NearestMedicineShop(props) {
    ...
    const { getMedicineStoreInfo } = useGlobal();

    useEffect(() => {
        setFetching(true);
        getMedicineStoreInfo(setFetching, setNearestInfo, 1000);
        const backAction = () => {
            navigation.goBack();
            return true;
        };
